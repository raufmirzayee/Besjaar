...
(() => {
  ...
  const announce=(message)=>{let el=qs('[data-besjaar-ui-live-region]');if(!el){el=document.createElement('p');el.className='besjaar-ui-visually-hidden';el.setAttribute('data-besjaar-ui-live-region','');el.setAttribute('role','status');el.setAttribute('aria-live','polite');document.body.appendChild(el)}el.textContent='';window.setTimeout(()=>{el.textContent=message},80)};
