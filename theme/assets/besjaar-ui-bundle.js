/* Besjaar UI 6.3 — bundle + cart add-ons */
(() => {
  const config=window.BesjaarUIConfig||{}, routes=config.routes||{}, currency=config.currency||'EUR';
  ...
  document.querySelectorAll('[data-besjaar-ui-bundle]').forEach(bundle=>{
    ...
    document.addEventListener('besjaar_ui:variant-change',event=>{const detail=event.detail||{};if(!detail.id)return;bundle.dataset.baseVariantId=String(detail.id);bundle.dataset.basePrice=String(detail.price||0);bundle.dataset.baseAvailable=detail.available?'true':'false';render()});
    button?.addEventListener('click',async()=>{
      ...
      const original=button.dataset.defaultLabel||button.textContent;button.disabled=true;button.setAttribute('aria-busy','true');button.textContent=button.dataset.addingLabel||'Adding…';if(status){status.textContent='';status.className='besjaar-ui-bundle-status'}
