/* Besjaar Besjaar Fusion 11.5.3 — small progressive enhancements only. */
(() => {
  'use strict';
  ...
  const initLightbox = (root) => {
    ...
    const thumbs = qsa('.besjaar-store-premium-pd__thumb', root);
  ...
  const init = (scope = document) => {
    ...
    qsa('.besjaar-store-premium-pd__details', scope).forEach(initProductAccordions);
...
(() => {
  ...
  const context = (window.BesjaarUIConfig && window.BesjaarUIConfig.analytics && window.BesjaarUIConfig.analytics.context) || {};
  ...
  const renderInto = (section) => {
    ...
    if (!items.length) {
      ...
      if (window.Shopify && window.Shopify.designMode) {
        ...
        note.className = 'besjaar-ui-copy';
    ...
    items.forEach((item) => {
      ...
      title.textContent = window.BesjaarUICatalogTitle ? window.BesjaarUICatalogTitle(item.t) : item.t;
