...
/* Besjaar Nova 3.0 — Product Q&A + Back-in-Stock + Retail Intelligence */
(() => {
  ...
  const config=window.NovaConfig||{}, routes=config.routes||{}, i18n=config.i18n||{}, commerce=config.commerce||{}, searchDiscovery=config.searchDiscovery||{}, catalog=config.catalog||{};
  ...
  window.NovaCatalogTitle=translateCatalogTitle;
  window.NovaCollectionTitle=translateCollectionTitle;
  ...
  const toast=(message)=>{let el=qs('.nova-toast');if(!el){el=document.createElement('div');el.className='nova-toast';el.setAttribute('role','status');document.body.appendChild(el)}el.textContent=message;el.classList.add('is-visible');clearTimeout(window.__novaToast);window.__novaToast=setTimeout(()=>el.classList.remove('is-visible'),2200)};
  ...
  const closeDrawers=()=>{if(!activeDrawer)return;qsa('.nova-drawer.is-open').forEach(el=>{el.classList.remove('is-open');el.setAttribute('aria-hidden','true');setInert(el,true)});backdrop?.classList.remove('is-open');document.body.style.overflow='';const focus=returnFocus;activeDrawer=null;returnFocus=null;focus?.focus?.()};
  ...
  qsa('.nova-account-menu').forEach(details=>{
  ...
  const recentSearchKey='besjaar_nova_recent_searches';
  ...
  document.addEventListener('click',event=>{const link=event.target.closest('[data-recent-search-link],.nova-search-shortcuts a,.nova-search-brands a');if(!link)return;try{const url=new URL(link.href,window.location.origin),term=url.searchParams.get('q');if(term)rememberSearch(term)}catch{}});

  const openSearch=(trigger)=>{if(!searchPanel)return;returnFocus=trigger||document.activeElement;searchPanel.classList.add('is-open');searchPanel.setAttribute('aria-hidden','false');setInert(searchPanel,false);searchToggles.forEach(b=>b.setAttribute('aria-expanded','true'));document.body.classList.add('nova-search-open');renderRecentSearches();setTimeout(()=>qs('[data-predictive-input]',searchPanel)?.focus(),20)};
  function closeSearch(){if(!searchPanel?.classList.contains('is-open'))return;searchPanel.classList.remove('is-open');searchPanel.setAttribute('aria-hidden','true');setInert(searchPanel,true);searchToggles.forEach(b=>b.setAttribute('aria-expanded','false'));document.body.classList.remove('nova-search-open');const f=returnFocus;returnFocus=null;f?.focus?.()}
  ...
  // Nova 6.1 — native Shopify predictive search. Uses the JSON endpoint already exposed by Shopify,
  ...
  const renderPredictive=(input,products,term)=>{
    ...
    list=list.slice(0,6);
    if(!list.length){
      box.innerHTML=`<div class="nova-predictive__empty">${escapeHtml(i18n.searchNoResults||'No matches yet')}</div><a class="nova-predictive__all" href="${escapeHtml(searchUrlForTerm(term))}">${escapeHtml(i18n.searchAll||'View all results')} →</a>`;
      if(box.id)input.setAttribute('aria-controls',box.id);
    ...
    const listboxId=`${box.id||'NovaPredictive'}-listbox`;
    input.setAttribute('aria-controls',listboxId);
    box.innerHTML=`<div class="nova-predictive__grid"><div class="nova-predictive__group nova-predictive__group--products" role="listbox" id="${escapeHtml(listboxId)}" aria-label="${escapeHtml(i18n.searchProducts||'Products')}"><h3 aria-hidden="true">${escapeHtml(i18n.searchProducts||'Products')}</h3>${list.map(product=>{
      ...
      return `<a class="nova-predictive__product" role="option" href="${escapeHtml(localizeInternalUrl(product?.url||'#'))}">${image?`<img src="${escapeHtml(image)}${String(image).includes('?')?'&':'?'}width=112" alt="" width="56" height="56" loading="lazy" decoding="async">`:'<span></span>'}<span class="nova-predictive__product-copy"><em>BESJAAR</em><strong>${escapeHtml(title)}</strong><span class="nova-predictive__meta">${price?`<small>${escapeHtml(price)}</small>`:''}${availability}</span></span><span aria-hidden="true">→</span></a>`;
    }).join('')}</div></div><a class="nova-predictive__all" href="${escapeHtml(searchUrlForTerm(term))}">${escapeHtml(i18n.searchAll||'View all results')} →</a>`;
  ...
  const cartRangeProducts=()=>{try{const node=qs('[data-nova-cart-range]');if(!node)return[];const parsed=JSON.parse(node.textContent||'[]');return Array.isArray(parsed)?parsed:[]}catch{return[]}};
  const cartAddonProducts=()=>{try{const node=qs('[data-nova-cart-addons]');if(!node)return[];const parsed=JSON.parse(node.textContent||'[]');return Array.isArray(parsed)?parsed:[]}catch{return[]}};
  const renderCartRange=cart=>{
    ...
    target.innerHTML=`<div class="nova-cart-upsell">
      <div class="nova-cart-upsell__header">
        <div class="nova-cart-upsell__label"><span>${escapeHtml(heading)}</span><small>${escapeHtml(copy)}</small></div>
        <button class="nova-cart-upsell__toggle" type="button" data-cart-upsell-toggle aria-controls="NovaCartUpsellBody" aria-expanded="${isOpen?'true':'false'}" aria-label="${escapeHtml(isOpen?collapseLabel:expandLabel)}">
          <span data-upsell-toggle-text>${escapeHtml(isOpen?collapseText:expandText)}</span>
          <i aria-hidden="true">${isOpen?'−':'+'}</i>
        </button>
      </div>
      <div class="nova-cart-upsell__body" id="NovaCartUpsellBody">
        <div class="nova-cart-upsell__rail">${picks.map(item=>`<div class="nova-cart-upsell__product"${useAddons?' data-addon="true"':''}>${item.image?`<a class="nova-cart-upsell__media" href="${escapeHtml(item.url)}"><img src="${escapeHtml(item.image)}" alt="${escapeHtml(translateCatalogTitle(item.title))}" loading="lazy"></a>`:''}<div class="nova-cart-upsell__copy"><a href="${escapeHtml(item.url)}"><strong>${escapeHtml(translateCatalogTitle(item.title))}</strong></a><span>${formatMoney(item.price,cart?.currency||currency)}</span></div><button class="nova-cart-upsell__add" type="button" data-cart-range-add="${escapeHtml(item.variantId)}">+ ${escapeHtml(i18n.quickAdd||'Add to cart')}</button></div>`).join('')}</div>
      </div>
      <button class="nova-cart-upsell__tab" type="button" data-cart-upsell-toggle aria-controls="NovaCartUpsellBody" aria-expanded="${isOpen?'true':'false'}" aria-label="${escapeHtml(isOpen?collapseLabel:expandLabel)}">
    ...
    const syncUpsellState=open=>{
      ...
      qsa('[data-cart-upsell-toggle]',target).forEach(toggle=>{
        ...
        if(toggle.classList.contains('nova-cart-upsell__toggle'))toggle.tabIndex=open?0:-1;
        ...
        if(toggle.classList.contains('nova-cart-upsell__tab')&&tabText)tabText.textContent=open?collapseText:expandText;
      });
      const upsellBody=qs('.nova-cart-upsell__body',target);
  ...
  document.addEventListener('click',async event=>{const button=event.target.closest('[data-card-quick-add]');if(!button)return;event.preventDefault();if(button.disabled)return;const variantId=button.dataset.cardQuickAdd;if(!variantId)return;const original=button.innerHTML;button.disabled=true;button.classList.add('is-loading');button.textContent=i18n.adding||'Adding…';let added=false;try{await addVariant(variantId,1);added=true}catch{const fallbackForm=button.closest('form.nova-card-quick-form');if(fallbackForm){HTMLFormElement.prototype.submit.call(fallbackForm);return}const card=button.closest('.nova-product-card,.nova-shower-card,.bk-product-card,.ba-shop-card');window.location.href=card?.querySelector('h3 a,h2 a,.nova-product-card__title a')?.href||routes.allProducts||'/collections/all';return}try{measure('quick_add_succeeded',{variantId:String(variantId),quantity:1,placement:'product_card'});toast(i18n.added||'Added to cart');openDrawer('cart',button)}catch{if(added){toast(i18n.added||'Added to cart');try{openDrawer('cart',button)}catch{}}}finally{button.disabled=false;button.classList.remove('is-loading');button.innerHTML=original}});
  ...
  const setCartLineBusy=(button,busy)=>{
    const line=button?.closest?.('.nova-cart-drawer__line');
    if(!line)return;
  ...
  const refreshCart=async()=>{
    ...
    body.innerHTML=`<div class="nova-empty nova-cart-loading"><span class="nova-cart-loading__dot" aria-hidden="true"></span>${escapeHtml(i18n.cartLoading||'Loading…')}</div>`;
    try{
      ...
      if(!cart.items.length){
        ...
        body.innerHTML=`<div class="nova-empty nova-cart-empty-state"><p>${escapeHtml(i18n.cartEmpty||'Your cart is empty.')}</p><a class="nova-button nova-button--primary" href="${escapeHtml(routes.allProducts||'/collections/all')}">${escapeHtml(i18n.shopProducts||'Shop products')}</a></div>`;
      ...
      body.innerHTML=cart.items.map(item=>{
        ...
        const priceHtml=`<div class="nova-cart-drawer__prices">${hasDiscount?`<del><span class="nova-visually-hidden">${escapeHtml(i18n.regularPrice||'Regular price')} </span>${formatMoney(original,currencyCode)}</del>`:''}${hasDiscount?`<span class="nova-visually-hidden">${escapeHtml(i18n.salePrice||'Sale price')} </span>`:''}<strong>${formatMoney(finalPrice,currencyCode)}</strong>${hasDiscount?`<em>${escapeHtml((i18n.savePercent||'Save %percent%').replace('%percent%',savingPct+'%'))}</em>`:''}</div>`;
        return `<div class="nova-drawer__item nova-cart-drawer__line" aria-busy="false">${item.image?`<a class="nova-cart-drawer__media" href="${escapeHtml(localizeInternalUrl(item.url))}" aria-label="${escapeHtml(itemTitle)}"><img src="${escapeHtml(item.image)}" alt="${escapeHtml(itemTitle)}" width="124" height="124" loading="lazy" decoding="async"></a>`:'<div class="nova-cart-drawer__media nova-cart-drawer__media--empty" aria-hidden="true"></div>'}<div class="nova-cart-drawer__line-main"><a href="${escapeHtml(localizeInternalUrl(item.url))}"><h3>${escapeHtml(itemTitle)}</h3></a>${variantMeta?`<small class="nova-cart-drawer__variant">${variantMeta}</small>`:''}${item.selling_plan_allocation?.selling_plan?.name?`<small class="nova-selling-plan-line">${escapeHtml(item.selling_plan_allocation.selling_plan.name)}</small>`:''}${properties?`<small class="nova-cart-drawer__properties">${properties}</small>`:''}</div><div class="nova-cart-drawer__line-side">${priceHtml}<div class="nova-line-quantity"><button type="button" data-line-minus="${escapeHtml(item.key)}" aria-label="${escapeHtml((i18n.decrease||'Decrease')+' '+itemTitle)}">−</button><span>${item.quantity}</span><button type="button" data-line-plus="${escapeHtml(item.key)}" aria-label="${escapeHtml((i18n.increase||'Increase')+' '+itemTitle)}">+</button></div></div><button class="nova-cart-drawer__remove" type="button" data-remove-line="${escapeHtml(item.key)}" aria-label="${escapeHtml((i18n.remove||'Remove')+' '+itemTitle)}">×</button></div>`
      }).join('');
      ...
      if(foot)foot.hidden=true;
      if(checkout){checkout.disabled=true;checkout.removeAttribute('aria-busy')}
      body.innerHTML=`<div class="nova-empty nova-cart-error-state"><p>${escapeHtml(i18n.cartError||'Cart could not be loaded.')}</p><div class="nova-cart-error-state__actions"><button class="nova-button nova-button--primary" type="button" data-cart-retry>${escapeHtml(i18n.retry||'Retry')}</button><a class="nova-button nova-button--ghost" href="${escapeHtml(routes.cart||'/cart')}">${escapeHtml(i18n.openCart||'Open cart')}</a></div></div>`;
...
/* Besjaar Nova 3.3 — Mobile Commerce & Shopify Markets OS */
(() => {
  const config = window.NovaConfig || {};
  ...
  const stage = productRoot?.querySelector('.nova-product-v16__media-stage');
  ...
  if (searchPanel && mobile.fullscreenSearch !== false) {
    const observer = new MutationObserver(() => {
      ...
      document.documentElement.classList.toggle('nova-mobile-search-open', open);
    });
    observer.observe(searchPanel, { attributes: true, attributeFilter: ['class'] });
  }
})();

/* ===== Nova 3.4.1 consolidated polish helpers ===== */
/* Besjaar Nova 3.4 — restrained motion and polish helpers */
(() => {
  const body = document.body;
  if (!body?.classList.contains('nova-polish-v34')) return;
  const design = window.NovaConfig?.design || {};
  const designMode = window.NovaConfig?.editor?.designMode === true || window.Shopify?.designMode === true;
  const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (designMode) body.classList.add('nova-design-mode');

  if (design.cardLift !== false) body.classList.add('nova-card-lift');
  if (design.stickyDesktopBuybox !== false) body.classList.add('nova-sticky-buybox');

  if (design.polishedMotion !== false && !reduced) {
    body.classList.add('nova-motion-enabled');
  }

  if (designMode || design.sectionReveal === false || design.polishedMotion === false || reduced || !('IntersectionObserver' in window)) return;

  const targets = [
    ...document.querySelectorAll('.shopify-section > .nova-section, .shopify-section > section'),
    ...document.querySelectorAll('.nova-card, .nova-product-card, .nova-promo-card, .nova-trust-item')
  ].filter((node, index, arr) => arr.indexOf(node) === index);

  targets.forEach((node, index) => {
    node.setAttribute('data-nova-polish-reveal','');
    node.setAttribute('data-nova-polish-reveal-delay', String(index % 4));
  });

  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (!entry.isIntersecting) return;
      entry.target.classList.add('is-nova-visible');
...
/* Besjaar 11.6 — delivery-date estimator (moved from nova-product.js so the cart
   drawer, cart page and home delivery section show computed dates on every page,
   not only on shower product pages). */
(() => {
  const config=window.NovaConfig||{}, delivery=config.delivery||{}, i18n=config.i18n||{};
