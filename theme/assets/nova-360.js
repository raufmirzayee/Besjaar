(() => {
  ...
  function init360(root) {
    ...
    if (root.dataset.nova360Ready === 'true') return;
    root.dataset.nova360Ready = 'true';

    const viewer = root.querySelector('[data-nova-360-viewer]');
    const image = root.querySelector('[data-nova-360-image]');
    const data = root.querySelector('[data-nova-360-frames]');
    ...
    const previous = root.querySelector('[data-nova-360-prev]');
    const next = root.querySelector('[data-nova-360-next]');
    const currentNode = root.querySelector('[data-nova-360-current]');
    const progress = root.querySelector('[data-nova-360-progress]');
    const playButton = root.querySelector('[data-nova-360-play]');
    const playLabel = root.querySelector('[data-nova-360-play-label]');
    const loadingNode = root.querySelector('[data-nova-360-loading]');
  ...
  function initAll(scope = document) {
    scope.querySelectorAll?.('[data-nova-360]').forEach(init360);
  }
