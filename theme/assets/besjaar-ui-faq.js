/* Besjaar UI 7.4.1 — dedicated FAQ search */
(() => {
  ...
  const init = (root) => {
    ...
    items.forEach((item, index) => {
      ...
      if (!item.id) item.id = `BesjaarUIFaqAnswer-${index + 1}`;
    });
  ...
  const boot = () => document.querySelectorAll('[data-besjaar-ui-faq-page]').forEach(init);
