/* Besjaar Nova 5.0 Phase 1 — product page runtime */
/* Besjaar 11.6: honor prefers-reduced-motion for programmatic scrolling. */
const novaScrollBehavior = () => (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) ? 'auto' : 'smooth';
/* Besjaar Nova 1.8 — Product Page Luxury Polish */
(() => {
  ...
  const dynamicCheckout = qs('.nova-product-v16__dynamic');
  ...
  const mediaStage = qs('.nova-product-v16__media-stage');
  const mediaSlides = qsa('[data-product-media]');
  const mediaThumbs = qsa('[data-product-media-thumb]');
  const swipeGalleryEnabled = root.classList.contains('nova-mobile-gallery-enabled');
  ...
  const updateMediaState = (mediaId) => {
    ...
    mediaThumbs.forEach((thumb) => {
      ...
      if (active) {
        ...
        if (isMobileSwipeGallery()) thumb.scrollIntoView({ behavior: novaScrollBehavior(), block: 'nearest', inline: 'center' });
      }
    });
  ...
  const switchMedia = (mediaId, { scroll = true } = {}) => {
    ...
    if (scroll && isMobileSwipeGallery()) {
      ...
      if (index >= 0) mediaStage.scrollTo({ left: index * mediaStage.clientWidth, behavior: novaScrollBehavior() });
    }
  };
  ...
  if (swipeGalleryEnabled && mediaStage && mediaSlides.length > 1) {
    ...
    const moveDesktopGallery = (direction) => {
      ...
      mediaThumbs[next]?.scrollIntoView?.({ behavior: novaScrollBehavior(), block: 'nearest', inline: 'center' });
    };
  ...
  const renderVariant = (variant, updateUrl = true) => {
    ...
    document.dispatchEvent(new CustomEvent('nova:variant-change', { detail: variant }));
  };
  ...
  document.querySelectorAll('[data-scroll-alternatives]').forEach(button => button.addEventListener('click', () => document.querySelector('[data-product-alternatives], [data-nova-recommendations][data-recommendation-intent="related"], [data-nova-recommendations]')?.scrollIntoView({ behavior: novaScrollBehavior(), block: 'start' })));
...
// Nova 2.3 — account menu interaction
(() => {
  const accountMenu = document.querySelector('.nova-account-menu');
...
// Nova 2.5 delivery-date estimator: moved to nova-core.js in Besjaar 11.6 so the
...
// Nova 3.1 — Promotions & Campaign Merchandising OS
(() => {
  const config=window.NovaConfig||{},i18n=config.i18n||{};
...
// Nova 5.6.9 — prevent duplicate oversized payment-badge widgets inside the buy box.
(() => {
  const root = document.querySelector('[data-product-root]');
  const buybox = root?.querySelector('[data-product-buybox]');
  if (!buybox) return;

  const nativePayments = buybox.querySelector('.nova-product-v16__payments');
  ...
  const removeDuplicatePaymentBadges = () => {
    ...
    candidates.forEach((candidate) => {
      if (candidate === buybox || candidate === nativePayments || candidate.closest('.nova-product-v16__payments')) return;
