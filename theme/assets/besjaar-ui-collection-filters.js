 ...
 *
 * After every successful swap the root fires `besjaar_ui:collection:updated`, so
  ...
  class CollectionResults {
    ...
    async render(url, { history = true, scrollToResults = false } = {}) {
      ...
      try {
        ...
        if (history) window.history.pushState({ besjaarUiCollection: true }, '', publicHref);
        if (scrollToResults) this.scrollToResults();

        this.root.dispatchEvent(
          new CustomEvent('besjaar_ui:collection:updated', { bubbles: true, detail: { url: publicHref } })
        );
      } catch (error) {
        ...
        console.error('[besjaar-ui-collection]', error);
