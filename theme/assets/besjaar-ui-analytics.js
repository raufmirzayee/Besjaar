/* Besjaar UI 7.3.6 — Analytics, Consent & Conversion Measurement OS
 ...
 */
(() => {
  const rootConfig = window.BesjaarUIConfig || {};
  ...
  const dialog = document.querySelector('[data-besjaar-ui-privacy-dialog]');
  const debugRoot = document.querySelector('[data-besjaar-ui-measurement-debug]');
  ...
  const updateDebug = () => {
    if (!debugRoot) return;
    const set = (key, value) => { const el = qs(`[data-besjaar-ui-debug="${key}"]`, debugRoot); if (el) el.textContent = value; };
  ...
  const publishShopify = (name, detail = {}) => {
    ...
    // BesjaarUI publishes only Besjaar-prefixed UI/custom events to avoid duplicate GA4 purchases or add_to_cart events.
    try {
  ...
  const dispatch = (name, detail = {}) => {
    ...
    document.dispatchEvent(new CustomEvent(`besjaar_ui:${name}`, { detail: payload }));
    publishShopify(name, detail);
    if (config.debug) console.info('[BesjaarUI 7.3.6]', name, detail);
  };
  ...
  const captureAttribution = () => {
    ...
    try {
      const stored = JSON.parse(sessionStorage.getItem('besjaar_ui_attribution_v32') || '{}');
      ...
      if (Object.keys(next).length) sessionStorage.setItem('besjaar_ui_attribution_v32', JSON.stringify(state.attribution));
  ...
  const readPrivacy = () => {
    ...
    captureAttribution();
    updateDebug();
    document.dispatchEvent(new CustomEvent('besjaar_ui:privacy-state', { detail: { ...state.privacy } }));
  ...
  const syncDialog = () => {
    ...
    Object.keys(map).forEach(key => {
      const input = qs(`[data-besjaar-ui-consent="${key}"]`, dialog);
      if (!input) return;
  ...
  const openPrivacy = async () => {
    ...
    await loadPrivacy();
    syncDialog();
    const status = qs('[data-besjaar-ui-privacy-status]', dialog); if (status) status.textContent = '';
    if (typeof dialog.showModal === 'function') dialog.showModal(); else dialog.setAttribute('open','');
    qs('[data-besjaar-ui-consent="preferences"]', dialog)?.focus();
  };
  const closePrivacy = () => { if (!dialog) return; if (typeof dialog.close === 'function') dialog.close(); else dialog.removeAttribute('open'); };

  qsa('[data-besjaar-ui-privacy-open]').forEach(button => button.addEventListener('click', openPrivacy));
  qsa('[data-besjaar-ui-privacy-close]').forEach(button => button.addEventListener('click', closePrivacy));
  ...
  qs('[data-besjaar-ui-privacy-save]', dialog || document)?.addEventListener('click', async event => {
    const button = event.currentTarget;
    const status = qs('[data-besjaar-ui-privacy-status]', dialog);
    const cp = window.Shopify?.customerPrivacy;
    ...
    const consent = {
      preferences: Boolean(qs('[data-besjaar-ui-consent="preferences"]', dialog)?.checked),
      analytics: Boolean(qs('[data-besjaar-ui-consent="analytics"]', dialog)?.checked),
      marketing: Boolean(qs('[data-besjaar-ui-consent="marketing"]', dialog)?.checked)
    };
  ...
  // Consent-gated storefront intent bridge. Nothing is transmitted by BesjaarUI.
  document.addEventListener('click', event => {
    ...
    const campaign = target.closest('[data-besjaar-ui-campaign]');
    if (campaign) dispatch('campaign_click', { campaign: campaign.dataset.besjaarUiCampaign || '', placement: campaign.dataset.besjaarUiPlacement || '' });
  });
  ...
  document.addEventListener('besjaar_ui:measurement', event => {
    if (!state.privacy.analytics) return;
