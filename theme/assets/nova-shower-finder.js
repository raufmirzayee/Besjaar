/* Besjaar Nova 5.7 — product-page shower model finder */
(() => {
  ...
  roots.forEach((root) => {
    ...
    const rank = () => {
      ...
      if (hasChoice && best) {
        ...
        if (resultCard) {
          ...
          const facts = [...best.querySelectorAll('.nova-shower-model-card__facts span')].map(item => item.textContent.trim()).filter(Boolean);
          if (resultFacts) {
        ...
        root.dispatchEvent(new CustomEvent('nova:finder-result', { bubbles: true, detail: { modelId: best.dataset.modelId || '', title, ...state } }));
      } else {
    ...
    questions.forEach((question) => {
      question.querySelectorAll('[data-finder-choice]').forEach((button) => {
        button.addEventListener('click', () => {
          ...
          rank();
          window.NovaMeasurement?.emit?.('product_finder_choice', { question: key, value });
        });
      });
    });
    ...
    reset?.addEventListener('click', () => {
      ...
      rank();
      window.NovaMeasurement?.emit?.('product_finder_reset');
    });

    root.addEventListener('click', (event) => {
      const link = event.target.closest('.nova-shower-model-card__footer a');
      if (!link) return;
      const card = link.closest('[data-finder-model]');
      window.NovaMeasurement?.emit?.('product_finder_model_click', {
...
/* Story engagement measurement: consent-gated through NovaMeasurement. */
(() => {
  ...
  const observer = new IntersectionObserver((entries) => {
    ...
    if (!entry) return;
    seen = true;
    window.NovaMeasurement?.emit?.('product_story_view');
    observer.disconnect();
  }, { threshold: [0.35] });
  observer.observe(story);

  const video = story.querySelector('[data-story-video] video');
  video?.addEventListener('play', () => window.NovaMeasurement?.emit?.('product_story_video_play'), { once: true });
})();
