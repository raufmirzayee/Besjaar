...
(() => {
  ...
  if (invitation && !getValidClaim() && !getStored(keys.closed) && !opened) invitation.hidden = false;
  document.querySelector('[data-offer-launch]')?.addEventListener('click', () => {
  ...
  document.querySelector('[data-offer-dismiss]')?.addEventListener('click', () => {
    if (invitation) invitation.hidden = true;
    setStored(keys.closed, { at: Date.now() });
  });
