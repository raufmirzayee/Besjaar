(() => {
  const selector='[data-nova-recommendations][data-recommendations-loaded="false"]';
  ...
  const loadSection=async section=>{
    ...
    try{
      ...
      const fresh=holder.querySelector(`[data-nova-recommendations][data-section-key="${safeKey}"]`) || holder.querySelector('[data-nova-recommendations]');
      ...
      // Recommendation HTML is injected after Nova's page-level reveal observer has run.
      ...
      document.dispatchEvent(new CustomEvent('nova:recommendations:loaded',{detail:{section:fresh,intent:fresh.dataset.recommendationIntent||''}}));
    }catch(error){
