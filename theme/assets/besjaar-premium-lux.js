/* Besjaar 11.3.0 — Premium Lux runtime.
   ...
   pulse. No reveal observers (nova-core.js owns those), no drawer or search
   logic, nothing the page depends on to function. */
(() => {
  const body = document.body;
  if (!body || !body.classList.contains('nova-polish-v34')) return;
  if (body.classList.contains('nova-editor-safe')) return;

  // Header depth on scroll. Mobile already has is-mobile-compact (nova-core.js);
  ...
  // Pulse cart-count bubbles when their number changes. nova-core.js only sets
  ...
  if (!body.classList.contains('nova-motion-enabled')) return;
  document.querySelectorAll('[data-cart-count]').forEach(node => {
    let timer = null;
    let last = node.textContent.trim();
    const observer = new MutationObserver(() => {
      // nova-core rewrites textContent on every drawer render; only a real
